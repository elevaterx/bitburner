...
export async function main(ns) {
  ...
  while (true) {
    ...
    const rsInfo = (proc) => { const rs = ns.getRunningScript(proc.pid); return { up: rs ? rs.onlineRunningTime * 1000 : 0, ram: rs ? rs.ramUsage * rs.threads : 0 }; };
    ...
    for (const j of WORKER_JOBS) {
      ...
      workerRows.push({
        kind: "worker", file: j.file, label: j.label, running: !!proc, up: info.up, ram: info.ram, status: j.key ? formatStatus(readStatus(ns, j.key), now) : null,
    ...
    ns.write("panel-data.txt", JSON.stringify({
      ...
      workers: workerRows.map((r) => ({ label: r.label, up: r.up, ram: r.ram, status: r.status })),
    }), "w");
...
function view(h, { modRows, workerRows, homeFree, homeMax, node, auto, pending }) {
  ...
  const workerBody = workerRows.map((r) => {
    ...
    const infoCell = h("span", { style: { color: "#bbb", flex: 1, minWidth: 80 } }, (r.running ? "up " + fmtTime(r.up) : "stopped") + (r.status ? "  " + r.status : ""));
