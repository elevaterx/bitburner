...
/** Ranking uses `rankTimeMs` when present -- the nominal hackTime floored at the practical
 *  scheduling minimum. ns.hack resolves via a plain window.setTimeout (NetscriptHelpers.tsx:469),
 *  so once the nominal time falls under what the event loop can actually turn around (the 200ms
 *  engine tick, React, and every other running script contend for it), going faster buys nothing.
 *  XP per op is (3 + 0.3*baseDifficulty) and does NOT depend on op time -- so below the floor the
 *  only thing that still pays is a HIGHER baseDifficulty. Without the floor the ranker chases
 *  ever-faster low-difficulty servers straight past the point where speed stops converting. */
export function rankTargets(cands, moneyUp = 1) {
  return cands
    .map((c) => ({
      ...c,
      rate: effectiveHackXpRate(c.baseDifficulty, c.rankTimeMs ?? c.hackTimeMs, c.chance ?? 1, moneyUp),
    }))
    .filter((c) => c.rate > 0)
...
export function planHost(freeGb, ramPerThread, want) {
  ...
  // SUPPORT CAP. weaken + grow together may not exceed this many threads on this host, so the
  // fill always reaches hack. Sizing support in ABSOLUTE threads is the trap: 4 grow instances of
  // 40 threads is 280GB, which is rounding error on a 524TB home and the ENTIRE host on a 27GB
  // network server. Without the cap, planHost hands the whole small host to the first grow
  // instance and hack silently gets zero -- the fleet degrades to the grow farm it replaced,
  // with nothing in the status line saying so. Caller passes a FRACTION of the host.
  const cap = Number.isFinite(want.supportCapThreads)
    ? Math.max(0, want.supportCapThreads)
    : Infinity;
  let support = 0;
  const room = (perThread) => Math.max(0, Math.min(Math.floor(gb / perThread), cap - support));

  const wt = Math.min(want.weaken || 0, room(ramPerThread.weaken));
  if (wt > 0) { out.weaken = wt; gb -= wt * ramPerThread.weaken; support += wt; }

  const gInst = Math.max(0, want.growInstances || 0);
  const gThreads = Math.max(1, want.growThreadsPerInstance || 1);
  for (let i = 0; i < gInst; i++) {
    const t = Math.min(gThreads, room(ramPerThread.grow));
    if (t <= 0) break;
    out.grow.push(t);
