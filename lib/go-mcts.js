...
let MARK = new Float64Array(0), LMARK = new Float64Array(0), STK = new Int32Array(0), POS = new Int32Array(0);
let GEN = 0;
function ensureScratch(n) {
  if (MARK.length < n) { MARK = new Float64Array(n); LMARK = new Float64Array(n); STK = new Int32Array(n); POS = new Int32Array(n); GEN = 0; }  // MARK/LMARK hold GEN stamps -> Float64 (exact to 2^53) so GEN can never overflow the stamp type
}
...
/** Sole liberty index of an in-atari group at i (its one empty point), or -1. */
function soleLiberty(arr, nb, i) {
  const color = arr[i], g1 = ++GEN; let top = 0; STK[top++] = i; MARK[i] = g1;
  while (top) { const c = STK[--top]; for (const j of nb[c]) { const v = arr[j]; if (v === EMPTY) return j; else if (v === color && MARK[j] !== g1) { MARK[j] = g1; STK[top++] = j; } } }
  return -1;
}

/** Would playing color at i leave its group in atari (<=1 lib) with no capture? Fast path: 2+ empty
 *  neighbours => safe; only the tight case clones. */
function isSelfAtari(arr, nb, size, i, color) {
  let empt = 0; for (const j of nb[i]) if (arr[j] === EMPTY) empt++;
  if (empt >= 2) return false;
  const c = arr.slice(); const r = playInPlace(c, nb, size, i, color);
  if (!r.ok) return true;
  return r.captured === 0 && groupLibsCount(c, nb, i) <= 1;
}

/** Local tactical response around the opponent's last move: capture an enemy group it left in atari, or
 *  save a friendly group it put in atari. Cheap (only inspects last move's neighbours). -1 if none. */
function tacticalMove(arr, nb, size, color, ko, last) {
  if (last < 0) return -1;
  const opp = other(color); const caps = [], saves = [];
  for (const j of nb[last]) {
    const v = arr[j]; if (v !== color && v !== opp) continue;
    const g = groupLibs(arr, nb, j);
    if (g.libs !== 1) continue;
    const lib = soleLiberty(arr, nb, j);
    if (lib < 0 || lib === ko) continue;
    (v === opp ? caps : saves).push(lib);
  }
  for (const i of caps) if (isLegal(arr, nb, i, color, ko)) return i;                     // capturing is always good
  for (const i of saves) if (isLegal(arr, nb, i, color, ko) && !isSelfAtari(arr, nb, size, i, color)) return i;
  return -1;
}

/** One HEAVY playout: local atari response (capture/save) first, else eye-safe, self-atari-avoiding
 *  random. Random playouts misjudge life-and-death; the tactical response makes group life realistic.
 *  Mutates arr. Returns the Black-view margin. */
export function randomPlayout(arr, nb, size, toMove, ko0, komi, rng, maxMoves) {
  ...
  const rm = (i) => { const p = POS[i]; if (p < 0) return; const lst = list[list.length - 1]; list[p] = lst; POS[lst] = p; list.pop(); POS[i] = -1; };
  const add = (i) => { if (POS[i] < 0) { POS[i] = list.length; list.push(i); } };

  let passes = 0, color = toMove, ko = ko0, moves = 0, lastMv = -1;
  while (passes < 2 && moves < maxMoves) {
    let played = tacticalMove(arr, nb, size, color, ko, lastMv);
    if (played < 0) {
      const L = list.length;
      for (let t = 0; t < 10 && L > 0; t++) {
        const i = list[(rng() * L) | 0];
        if (i !== ko && !isEyeFor(arr, nb, size, i, color) && isLegal(arr, nb, i, color, ko) && !isSelfAtari(arr, nb, size, i, color)) { played = i; break; }
      }
      if (played < 0) {
        for (let p = 0; p < list.length; p++) { const i = list[p]; if (i !== ko && !isEyeFor(arr, nb, size, i, color) && isLegal(arr, nb, i, color, ko) && !isSelfAtari(arr, nb, size, i, color)) { played = i; break; } }
      }
    }
    if (played < 0) { passes++; color = other(color); lastMv = -1; continue; }
    passes = 0;
    const r = playInPlace(arr, nb, size, played, color);
    rm(played);
    if (r.capturedStones) for (const s of r.capturedStones) add(s);
    ko = r.ok ? r.ko : -1; lastMv = played; color = other(color); moves++;
  }
