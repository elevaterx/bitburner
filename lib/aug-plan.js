...
export function selectRound(cands, money, opts = {}) {
  ...
  for (const r of bundled) {
    ...
    if (orderedCost(orderWithPrereqs(trial), mult) <= effBudget) {
      chosen = trial;
  ...
  for (;;) {
    ...
    // Drop the worst -- but never orphan a dependent. An aug that something else in the basket needs
    // cannot be removed on its own; removing it would leave a purchase the game refuses outright, and
    // silently pricing an impossible round is exactly the class of bug this file keeps stamping out.
    // Skip to the next-worst instead: if the chain is genuinely bad the DEPENDENT prices out first
    // (it sits at a later slot), and once it goes the prereq becomes droppable on the next iteration.
    const names = new Set(ordered.map((c) => c.aug));
    const needed = new Set();
    for (const c of ordered) for (const r of c.prereqs || []) if (names.has(r)) needed.add(r);
    const order = perValue
      .map((v, i) => ({ v, i }))
      .filter((x) => !needed.has(ordered[x.i].aug))
      .sort((a, b) => b.v - a.v);
    if (!order.length) break;
    if (!(order[0].v > best * cutoff)) break;
    chosen = ordered.filter((_, i) => i !== order[0].i);
  }
