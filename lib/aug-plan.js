...
import { hackMoneyIndex } from "./node-policy.js";
 ...
 *
 *  THE FACTION FILTER IS NOT OPTIONAL. An earlier version measured the gap across ALL candidates and
 *  divided it by the GANG's rep income, reasoning that a gang faction sells the entire augmentation
 *  table so the gang is always an option. That is false. BN3 sets GangUniqueAugs 0.5 and
 *  getFactionAugmentationsFiltered (FactionHelpers.tsx:172-199) drops faction-unique augs on a SEEDED
 *  coin flip. Live, "The Black Hand" lost that flip, so it was purchasable only from The Black Hand
 *  faction -- rep 38,961 against 300,000 required, with NOTHING feeding it. Dividing that 261k gap by
 *  the gang's 4,677/s produced a confident "57 seconds" for something the gang cannot close at all.
 *
 *  So: pass `factions` -- the set whose rep the income actually feeds -- and only gaps there count.
 *  Omit it and every candidate counts, which is the old behaviour and correct only when the income
 *  is faction-agnostic. */
export function maxRepGap(cands, factions = null) {
  let max = 0;
  for (const c of cands || []) {
    if (!c || !hasHackingValue(c.stats)) continue;
    if (factions && !factions.has(c.faction)) continue;
...
export function nodeWeights(ctx = {}, base = BASE_WEIGHTS) {
  // The PRODUCT is defined once, in lib/node-policy.js, because boot.js and sing.js ask the same
  // question with different thresholds. This keeps aug-plan's CONTINUOUS weight (a multiplier is
  // worth a fraction of vanilla, not nothing) while sharing the definition it is a fraction of.
  const w = moneyFarmWeight(
    hackMoneyIndex({ ScriptHackMoneyGain: ctx.scriptHackMoneyGain, ServerMaxMoney: ctx.serverMaxMoney }),
    !!ctx.moneyFarmRunning);
