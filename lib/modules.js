...
export const MODES = {
  ...
  // Trader has no live control file -- the liquidation path is an ARG (`trader.js sellall`) that
  // sells every position and exits. That makes `relaunch` the correct mode type here, and it is also
  // the SAFE one: relaunch does scriptKill-then-run, so the running trader is stopped BEFORE the
  // sell-all instance starts. Launching `trader.js sellall` by hand while trader is up would leave
  // two processes racing -- one liquidating, one re-buying.
  // Do this before installing augmentations: Prestige.ts:168 calls initStockMarket() inside
  // prestigeAugmentation, which deletes every Stock object and rebuilds it with playerShares = 0.
  // Positions are DESTROYED, not sold -- you get nothing back.
  "trader.js": { type: "relaunch", options: [
    { label: "Trade", args: [] },
    { label: "SELL ALL", args: ["sellall"], danger: true },
  ] },
  // Suspends ONLY sing.js's faction-work phase. A live control file, not a relaunch: sing.js is
  // 56GB and restarting it drops its backdoor/program progress for the loop. Sticky across restarts
  // by design -- sing.js reads this file and never rewrites it.
  "sing.js": { type: "write", file: "sing-ctl.txt", options: [
    { label: "Work", content: "work" },
    { label: "No work", content: "nowork", danger: true },
  ] },
