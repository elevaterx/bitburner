...
export const MODES = {
  ...
  "go.js": { type: "relaunch", options: [
    ...
    // game, and it multiplies raw hacking skill. resetBoardState exempts this opponent from the
    // [5,7,9,13] size whitelist, and getDifficultyMultiplier is (komi+0.5)*0.25 = 2.5 regardless
    // of size -- so a small board keeps games fast without giving up the multiplier.
    { label: "W0R1D 5x5",   args: ["--opponent", "????????????", "--no-cycle", "--size", "5"] },
    { label: "W0R1D 19x19", args: ["--opponent", "????????????", "--no-cycle", "--size", "19"] },
  ] },
};
